...
def taint_system(system_id: str, base_url: str, token: str):
    """
    Taints an agent system using a JWT for authentication.

    Args:
        system_id: The ID of the agent system to taint.
        base_url: The base URL of the server.
        token: The JWT for authentication.
    """
    taint_url = urljoin(base_url, f"/api/systems/taint/{system_id}")
    
    print(f"\nAuthenticated. Attempting to taint system ID '{system_id}' at '{taint_url}'...")
    start_time = time.time()

    try:
        headers = {'Authorization': f'Bearer {token}'}
        
        # Taint is a POST request as it changes the state of the resource.
        response = requests.post(taint_url, headers=headers, timeout=60)
        
        response.raise_for_status()

    except requests.exceptions.RequestException as e:
        print(f"\nAn error occurred during the taint operation: {e}")
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python taint.py <system_id> <base_url> <username> <password>")
        print("Example: python taint.py 12345 http://localhost:8080 my-user my-pass")
        sys.exit(1)
    
    system_id_to_taint, base_url, username, password = sys.argv[1:5]
    ...
    taint_system(system_id_to_taint, base_url, auth_token)